...
func DrawOneGlyph(glyph_name string, s Settings) image.Image {
	// Initialize the graphic context on an RGBA image
	glyph_image := image.NewRGBA(image.Rect(0, 0, s.ImageSize, s.ImageSize))
	...
	gc.SetLineWidth(s.HexagonLineWidth)
	drawHexagon(gc, s)

	gc.SetLineWidth(s.NodeLineWidth)
	drawNodeCircles(gc, s)

	gc.SetLineWidth(s.GlyphEdgeWidth)
...
func drawNodeCircles(gc draw2d.GraphicContext, s Settings) {
	for _, node_char := range "0123456789a" {
		xy := nodeXy(byte(node_char), s.Centre, s.Centre, s.CodeRadius)
		draw2dkit.Circle(gc, xy.X, xy.Y, s.NodeSize)
		gc.FillStroke()
	}
}

// draw_hexagon draws a hexagonal border around the glyph pad.
func drawHexagon(gc draw2d.GraphicContext, s Settings) {
	for n := 0; n <= 6; n++ {
		// [FIXME] 100,100,100 is hardcoded to 200px image size.
		xy := floatPolar{Mag: 1.00, Deg: float64(n)*60.0 + 30.0}.xy().moveAndScale(s.Centre, s.Centre, s.HexagonRadius)
		if n == 0 {
			gc.MoveTo(xy.X, xy.Y)
		} else {
			gc.LineTo(xy.X, xy.Y)
...
func drawEdge(gc draw2d.GraphicContext, node_pair string, s Settings) {
	xy1 := nodeXy(node_pair[0], s.Centre, s.Centre, s.CodeRadius)
	xy2 := nodeXy(node_pair[1], s.Centre, s.Centre, s.CodeRadius)
	...
	draw2dkit.Circle(gc, xy1.X, xy1.Y, s.NodeSize) //GLYPH_EDGE_WIDTH/2)
	gc.Fill()
	draw2dkit.Circle(gc, xy2.X, xy2.Y, s.NodeSize) //GLYPH_EDGE_WIDTH/2)
	gc.Fill()
}
...
func drawEdges(gc draw2d.GraphicContext, edge_list string, s Settings) {
	if len(edge_list) == 0 {
		return
	}
	...
	for n := 0; n < edge_pairs; n++ {
		drawEdge(gc, edge_list[2*n:2*n+2], s)
	}
}